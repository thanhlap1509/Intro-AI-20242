...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        # weights
        self.W_f = nn.Parameter(torch.nn.init.xavier_uniform_(torch.zeros(hidden_size, hidden_size + input_size)))
        self.b_f = nn.Parameter(torch.zeros(hidden_size, 1))
        
        self.W_i = nn.Parameter(torch.nn.init.xavier_uniform_(torch.zeros(hidden_size, hidden_size + input_size)))
        self.b_i = nn.Parameter(torch.zeros(hidden_size, 1))
        
        self.W_c = nn.Parameter(torch.nn.init.xavier_uniform_(torch.zeros(hidden_size, hidden_size + input_size)))
        self.b_c = nn.Parameter(torch.zeros(hidden_size, 1))
        
        self.W_o = nn.Parameter(torch.nn.init.xavier_uniform_(torch.zeros(hidden_size, hidden_size + input_size)))
        self.b_o = nn.Parameter(torch.zeros(hidden_size, 1))
        
        # Output layer
        self.W_y = nn.Parameter(torch.nn.init.xavier_uniform_(torch.zeros(output_size, hidden_size)))
    ...
    def forward(self, X):
        ...
        hidden_states = torch.tensor(hidden_states, dim=0)
